...
Um problema pertence à NP se a for possível verificar a validade de uma solução em tempo polimonial.\cite{SIPSER}\cite{Cook:1971:CTP:800157.805047bnh}.

Por mais que exista a dificuldade teórica supracitada, isso não impediu a evolução dos Sistemas Operacionais, os quais foram capazes de fazer escalonamento de vários processos mesmo na época de CPUs tinham apenas um núcleo. Popularizando dessa forma os computadores pessoais na década de 80.

Atualmente, as CPUs possuem vários núcleos, e são capazes de ter mais de um contexto carregado por núcleo, vide Ryzen™ Threadripper™\cite{Ryzen}. O que faz com que seja necessário que o processo de escalonamento leve em consideração como o mesmo será distribuído(ou não) entre os núcleos.
...
Esta monografia focará no último tópico citado. O qual recentemente presencioua ascenção do uso de unidades de processamento gráfico(GPU, do inglês) para processamento de propósito geral(GPGPU) 

E mais recentemente, as unidades de processamento gráfico(GPUs) ascenderam como opção para processamento paralelo.
